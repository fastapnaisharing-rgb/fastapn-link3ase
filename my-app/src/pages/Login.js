...
function Login() {
  ...
  // ── Maintenance banner: ดึงสถานะ + ข้อความจริงจาก backend ──────────────
  const [maintenanceOn, setMaintenanceOn] = useState(false);
  const [maintenanceMsg, setMaintenanceMsg] = useState('');
  useEffect(() => {
    const checkMaintenance = async () => {
      try {
        const res = await fetch(`${API}/api/public/maintenance`);
        const data = await res.json();
        setMaintenanceOn(String(data.maintenance_mode).toLowerCase() === 'true');
        setMaintenanceMsg(data.maintenance_message || '');
      } catch (e) { console.error('check maintenance:', e); }
    };
    checkMaintenance();
  }, []);
  ...
  const S = {
    page: { minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundImage: 'url("data:image/svg+xml;base64,")', backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat' },
    card: { background: '#f0f3f7', borderRadius: '16px', padding: '40px', width: '380px', boxShadow: 'none', border: '1px solid #c8d2de', position: 'relative', overflow: 'hidden' },
    ...
    tabs: { display: 'flex', background: '#e2e8f0', borderRadius: '8px', padding: '4px', marginBottom: '24px' },
    ...
    input: { width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #d0d8e4', fontSize: '14px', marginBottom: '16px', outline: 'none', boxSizing: 'border-box', background: '#f7f9fb' },
    btn: { width: '100%', padding: '12px', background: '#5b9e9a', color: 'white', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '600', cursor: 'pointer', marginTop: '4px', letterSpacing: '1px' },
  ...
  return (
    <>
      {maintenanceOn && (
        <>
          <style>{`@keyframes fastapnMarquee { from { transform: translateX(0); } to { transform: translateX(-100%); } }`}</style>
          <div style={{ background: '#FAC775', color: '#412402', overflow: 'hidden', whiteSpace: 'nowrap', height: '32px', display: 'flex', alignItems: 'center' }}>
            <div style={{ display: 'inline-block', paddingLeft: '100%', animation: 'fastapnMarquee 18s linear infinite', fontSize: '13px', fontWeight: 500 }}>
              ⚠️ {maintenanceMsg || 'ระบบกำลังปิดปรับปรุงบางส่วน อาจใช้งานบางเมนูไม่ได้ชั่วคราว'}
            </div>
          </div>
        </>
      )}
