    ...
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999 }}>
      <div style={{ background: 'white', borderRadius: '10px', padding: '20px', width: isMobile ? '95vw' : '90vw', maxWidth: '1100px', maxHeight: '88vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: '11px', color: '#888' }}>จะ Import เฉพาะ ➕ New และ 🔄 Update รวม <strong style={{ color: '#1a3a5c' }}>{confirmCount} รายการ</strong> / ข้าม ✅ No Change และ ⚠️ Duplicate</span>
...
function BusinessUnit({ activeSubTab, onSubTabChange }) {
  ...
  const INFO_FIELDS = ['BU','THAI COMPANY NAME','ENGLISH COMPANY NAME','TAX ID','PREPARE BY','DEPARTMENT','COMPANY CODE','VAT %','Last Rate (%)','BOOK','SEGMENT3','AP GRT Control','username','last_update'];
  const INFO_KEY = 'TAX ID';
  const INFO_COMBO = ['BU','DEPARTMENT','BOOK','AP GRT Control'];
  const INFO_EDIT = [['BU','BU'],['THAI COMPANY NAME','Thai Company Name'],['ENGLISH COMPANY NAME','English Company Name'],['TAX ID','Tax ID'],['PREPARE BY','Prepare By'],['DEPARTMENT','Department'],['COMPANY CODE','Company Code'],['VAT %','VAT %'],['Last Rate (%)','Last Rate (%)'],['BOOK','Book'],['SEGMENT3','Segment3'],['AP GRT Control','AP GRT Control']];
  const INFO_COLUMNS = [
  ...
  const BRANCH_FIELDS = ['Branch Code','Branch Direct','Branch Allocate','BU Code','Company for Show in Report Display','Simple Company','BU-TaxID','BU-Branch','Simple Brand Code','%','DB(%)','CPC','Branch Address','Group-P','BU','Status','Inactive Date','username','last_update'];
  ...
  const handleInfoExportSelected = () => exportToExcel(infoItems.filter(i => infoSelected.includes(i.id)), INFO_FIELDS.filter(f => !['username','last_update'].includes(f)), 'CompanyList', 'CompanyList');
  const handleInfoExportAll = () => exportToExcel(filteredInfo, INFO_FIELDS.filter(f => !['username','last_update'].includes(f)), 'CompanyList', 'CompanyList');
  const handleBranchExportSelected = () => exportToExcel(branches.filter(b => branchSelected.includes(b.id)), BRANCH_FIELDS.filter(f => !['username','last_update'].includes(f)), 'BranchList', 'BranchList');
  const handleBranchExportAll = () => exportToExcel(filteredBranch, BRANCH_FIELDS.filter(f => !['username','last_update'].includes(f)), 'BranchList', 'BranchList');

  const doInfoSave = async (form) => {
    const data = { ...form, username: userName || currentUser?.email || '', last_update: getTimestamp() };
    if (infoEditId) await updateDoc(doc(db, 'CompanyList', infoEditId), data);
    else await addDoc(collection(db, 'CompanyList'), data);
    setShowInfoForm(false); setInfoEditId(null); setInfoForm(emptyInfoForm());
    invalidate('CompanyList');
    await fetchInfo();
  };
  ...
  const handleRateConfirm = async () => {
    ...
    for (let i = 0; i < rateConfirmData.affectedIds.length; i += 500) {
      const batch = writeBatch(db);
      rateConfirmData.affectedIds.slice(i, i + 500).forEach(id => { batch.update(doc(db, 'BranchList', id), { '%': rateConfirmData.newRate, username: userName || currentUser?.email || '', last_update: getTimestamp() }); });
      await batch.commit();
    }
    setShowRateConfirm(false); setRateConfirmData(null);
    invalidate('BranchList');
    await fetchBranch();
  };
  ...
  const handleInfoDelete = async (id) => {
    if (window.confirm('ต้องการลบรายการนี้?')) {
      await deleteDoc(doc(db, 'CompanyList', id));
  ...
  const handleInfoBulkDelete = async () => {
    ...
    const batch = writeBatch(db); infoSelected.forEach(id => batch.delete(doc(db, 'CompanyList', id)));
    await batch.commit(); setInfoSelected([]);
    invalidate('CompanyList');
    await fetchInfo();
  };

  const handleInfoDownloadTemplate = () => { const ws = XLSX.utils.aoa_to_sheet([INFO_FIELDS.filter(f => !['username','last_update'].includes(f))]); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, 'CompanyList'); XLSX.writeFile(wb, 'CompanyList_Template.xlsx'); };
  ...
  const handleInfoConfirmImport = async () => {
    ...
    try {
      ...
      for (let i = 0; i < newRows.length; i += 500) {
        const batch = writeBatch(db);
        newRows.slice(i,i+500).forEach(row => { const d = doc(collection(db,'CompanyList')); const data={}; INFO_FIELDS.forEach(k => { if(k==='username') data[k]=userName||currentUser?.email||''; else if(k==='last_update') data[k]=getTimestamp(); else data[k]=String(row[k]??''); }); batch.set(d,data); });
        await batch.commit();
      }
      for (let i = 0; i < updateRows.length; i += 500) {
        const batch = writeBatch(db);
        updateRows.slice(i,i+500).forEach(row => { const data={}; INFO_FIELDS.forEach(k => { if(k==='username') data[k]=userName||currentUser?.email||''; else if(k==='last_update') data[k]=getTimestamp(); else data[k]=String(row[k]??''); }); batch.update(doc(db,'CompanyList',row._existingId),data); });
        await batch.commit();
      }
  ...
  const handleBranchDetailSave = async () => {
    ...
    await updateDoc(doc(db, 'BranchList', branchDetailItem.id), { ...branchDetailForm, username: userName || currentUser?.email || '', last_update: getTimestamp() });
  ...
  const handleBranchNewSave = async () => {
    ...
    await addDoc(collection(db, 'BranchList'), { ...branchNewForm, username: userName || currentUser?.email || '', last_update: getTimestamp() });
  ...
  const handleBranchDelete = async (id) => {
    if (window.confirm('ต้องการลบรายการนี้?')) {
      await deleteDoc(doc(db, 'BranchList', id));
  ...
  const handleBranchBulkDelete = async () => {
    ...
    const batch = writeBatch(db); branchSelected.forEach(id => batch.delete(doc(db, 'BranchList', id)));
    await batch.commit(); setBranchSelected([]);
    invalidate('BranchList');
    await fetchBranch();
  };

  const handleBranchDownloadTemplate = () => { const ws = XLSX.utils.aoa_to_sheet([BRANCH_FIELDS.filter(f => !['username','last_update'].includes(f))]); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, 'BranchList'); XLSX.writeFile(wb, 'BranchList_Template.xlsx'); };
  ...
  const handleBranchConfirmImport = async () => {
    ...
    try {
      ...
      for (let i = 0; i < newRows.length; i += 500) {
        const batch = writeBatch(db);
        newRows.slice(i,i+500).forEach(row => { const d=doc(collection(db,'BranchList')); const data={}; BRANCH_FIELDS.forEach(k => { if(k==='username') data[k]=userName||currentUser?.email||''; else if(k==='last_update') data[k]=getTimestamp(); else data[k]=String(row[k]??''); }); batch.set(d,data); });
        await batch.commit();
      }
      for (let i = 0; i < updateRows.length; i += 500) {
        const batch = writeBatch(db);
        updateRows.slice(i,i+500).forEach(row => { const data={}; BRANCH_FIELDS.forEach(k => { if(k==='username') data[k]=userName||currentUser?.email||''; else if(k==='last_update') data[k]=getTimestamp(); else data[k]=String(row[k]??''); }); batch.update(doc(db,'BranchList',row._existingId),data); });
        await batch.commit();
      }
  ...
  const S = {
    ...
    inputError: { padding: '7px 10px', borderRadius: '6px', border: '1px solid #e74c3c', fontSize: '13px', width: '100%', marginBottom: '4px', boxSizing: 'border-box' },
        ...
        {BRANCH_EDIT.map(([key, label]) => {
          ...
          return (
            <div key={key} style={{ marginBottom: '4px' }}>
              ...
              {editMode ? (
                key==='Inactive Date' ? <input type="date" style={hasError ? S.inputError : S.input} value={form[key]} onChange={e => { setForm({...form,[key]:e.target.value}); setError(''); }} />
                ...
                : <input style={hasError ? S.inputError : S.input} value={form[key]} onChange={e => { setForm({...form,[key]:e.target.value}); setError(''); }} />
              ) : <div style={S.inputReadonly}>{key==='Status' ? statusBadge(form[key]) : (form[key]||'-')}</div>}
              {hasError && <div style={{ fontSize: '11px', color: '#e74c3c', marginBottom: '4px' }}>กรุณากรอก{label}</div>}
            </div>
          );
        })}
      </div>
  ...
  const renderInfoModal = () => showInfoForm && (
    <div style={S.overlay}>
      <div style={S.modal}>
  ...
  const renderBranchDetailModal = () => showBranchDetail && branchDetailItem && (
    <div style={S.overlay}>
      <div style={S.modal}>
        ...
        {!branchDetailEditMode && (
          <div style={{ padding: '0 20px 16px', borderTop: '0.5px solid #f0f0f0', marginTop: '4px' }}>
            <div style={{ display: 'flex', gap: '16px', paddingTop: '12px' }}>
              <div style={{ flex: 1 }}><div style={{ fontSize: '11px', color: '#888' }}>Username</div><div style={{ fontSize: '12px', color: '#555', marginTop: '2px' }}>{branchDetailItem['username']||'-'}</div></div>
              <div style={{ flex: 1 }}><div style={{ fontSize: '11px', color: '#888' }}>Last Update</div><div style={{ fontSize: '12px', color: '#555', marginTop: '2px' }}>{formatLastUpdate(branchDetailItem['last_update'])}</div></div>
            </div>
          </div>
        )}
      </div>
  ...
  const renderBranchNewModal = () => showBranchNew && (
    <div style={S.overlay}>
      <div style={S.modal}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #f0f0f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
  ...
  const renderRateConfirmModal = () => showRateConfirm && rateConfirmData && (
        ...
        <div style={{ background: '#f8f9fa', borderRadius: '8px', padding: '16px', marginBottom: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '20px', marginBottom: '12px' }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '11px', color: '#888', marginBottom: '4px' }}>Rate เดิม → Last Rate (%)</div>
              <div style={{ fontSize: '22px', fontWeight: '600', color: '#791F1F' }}>{rateConfirmData.oldRate}%</div>
            </div>
            <div style={{ fontSize: '22px', color: '#888' }}>→</div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '11px', color: '#888', marginBottom: '4px' }}>Rate ใหม่ → VAT %</div>
              <div style={{ fontSize: '22px', fontWeight: '600', color: '#27500A' }}>{rateConfirmData.newRate}%</div>
            </div>
          </div>
          <div style={{ textAlign: 'center', fontSize: '12px', color: '#555', borderTop: '0.5px solid #e8e8e8', paddingTop: '12px' }}>
            จะอัปเดต <strong style={{ color: '#1a3a5c' }}>{rateConfirmData.branchCount} สาขา</strong> ที่มี Tax ID: {rateConfirmData.taxId}
          </div>
        </div>
        <div style={{ display: 'flex', gap: '8px' }}>
    ...
    <div style={S.container}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 0', margin: '4px 0', flexShrink: 0, flexWrap: isMobile ? 'wrap' : 'nowrap', gap: isMobile ? '6px' : '0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flex: isMobile ? '1 1 100%' : '1', minWidth: 0 }}>
          {tab==='info' ? (
            <input placeholder={isMobile ? 'Search...' : 'Search BU, Company Name, Tax ID...'} value={infoSearch} onChange={e => setInfoSearch(e.target.value)}
              style={{ padding: '5px 10px', borderRadius: '6px', border: '0.5px solid #ddd', fontSize: '12px', width: isMobile ? '100%' : isTablet ? '180px' : '240px' }} />
          ) : (
            <>
              <input placeholder={isMobile ? 'Search...' : 'Search Branch Code, Company, Address...'} value={branchSearch} onChange={e => { setBranchSearch(e.target.value); setBranchTaxFilter(''); }}
                style={{ padding: '5px 10px', borderRadius: '6px', border: '0.5px solid #ddd', fontSize: '12px', width: isMobile ? '100%' : isTablet ? '180px' : '240px' }} />
              {branchTaxFilter && !isMobile && (
                <span style={{ fontSize: '11px', background: '#e8f0fb', color: '#1a3a5c', padding: '3px 10px', borderRadius: '20px', display: 'flex', alignItems: 'center', gap: '4px', whiteSpace: 'nowrap' }}>
                  Tax ID: {isTablet ? '...' : branchTaxFilter}
                  <span style={{ cursor: 'pointer', fontWeight: 'bold' }} onClick={() => setBranchTaxFilter('')}>×</span>
                </span>
              )}
            </>
          )}
        ...
        {tab==='branch' && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flexShrink: 0 }}>
            ...
            <select value={branchPageSize} onChange={e => { setBranchPageSize(Number(e.target.value)); setBranchPage(1); }}
                  ...
                  <tr key={item.id} style={{ background: infoSelected.includes(item.id) ? '#f0f7ff' : 'white' }}>
                    ...
                    <td style={S.tdCenter}>
                      <div style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                        ...
                        {isAdmin && <>
                          <button onClick={() => handleInfoEdit(item)} style={S.iconBtn('#555','#f5f5f5','#ddd')}>✏️</button>
                          <button onClick={() => handleInfoDelete(item.id)} style={S.iconBtn('#791F1F','#FCEBEB','#f7c1c1')}>🗑️</button>
                        </>}
                      </div>
                    </td>
                  </tr>
      ...
      {renderInfoModal()}
      {renderBranchDetailModal()}
      {renderBranchNewModal()}
      {renderRateConfirmModal()}
