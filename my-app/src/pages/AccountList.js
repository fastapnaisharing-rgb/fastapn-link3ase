...
function AccountList() {
  ...
  const handleSave = async () => {
    if (editId) {
      await updateDoc(doc(db, 'accounts', editId), form);
    } else {
      await addDoc(collection(db, 'accounts'), form);
    }
    setShowForm(false);
    setEditId(null);
        ...
        <tbody>
          {filtered.map(a => (
            <tr key={a.id}>
              ...
              <td style={S.td}><span style={{ background: a.status === 'Active' ? '#EAF3DE' : '#FCEBEB', color: a.status === 'Active' ? '#27500A' : '#791F1F', padding: '2px 8px', borderRadius: '20px', fontSize: '11px' }}>{a.status}</span></td>
              <td style={S.td}>
