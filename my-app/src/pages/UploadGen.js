...
function DocumentCenter({ jumpToSetupToken, returnPage, onBackToCaller } = {}) {
    ...
    try {
      ...
      for (const att of newThreadAttachments) {
        ...
        await fetch(`${apiBase}/api/file-storage/upload-image`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
          body: JSON.stringify({ module: 'support-feedback', refId: data.thread.id, fileBase64: base64 }),
        });
      }
  ...
  const supportCounts = {
    ...
    in_process: supportThreads.filter(t => t.status === 'in_process').length,
    resolved: supportThreads.filter(t => t.status === 'resolved').length,
  };
  const filteredSupportThreads = supportThreads.filter(t => {
    if (t.status !== supportStatusTab) return false;
  ...
  // ── Unread: เฉพาะ Tab In process — เทียบ last_activity_at กับ read_at ของฝั่งตัวเอง ──
  const isThreadUnread = (t) => {
    if (t.status !== 'in_process' || !t.last_activity_at) return false;
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'10px' }}>
          <div style={{ display:'flex', gap:'6px' }}>
            {[['new','New'],['in_process','In process'],['resolved','Resolve']].map(([key,label]) => (
...
// MARKER_SUPPORT_LIST_INBOX_STYLE_V1
              ...
              {sortedFilteredSupportThreads.map((t, i) => {
                ...
                return (
                  <div key={t.id} onClick={()=>openThreadDetail(t.id)}
                    style={{ display:'flex', alignItems:'center', gap:'10px', padding:'10px 14px', borderBottom: i<sortedFilteredSupportThreads.length-1?'0.5px solid #f0f0f0':'none', cursor:'pointer', background: unread?'#E6F1FB':'white' }}>
      ...
      {showNewThreadForm && (
        <div style={{ position:'fixed', top:0, left:0, right:0, bottom:0, background:'rgba(0,0,0,0.4)', zIndex:9999, display:'flex', alignItems:'center', justifyContent:'center' }}>
          <div style={{ background:'white', borderRadius:'12px', width:'520px', maxWidth:'90vw', maxHeight:'85vh', overflowY:'auto', padding:'24px' }}>
            <div style={{ fontSize:'16px', fontWeight:'600', color:'#1a3a5c', marginBottom:'16px' }}>ตั้งกระทู้ใหม่</div>
            ...
            <div style={{ marginBottom:'12px' }}>
              ...
              <textarea value={newThreadBody} onChange={e=>setNewThreadBody(e.target.value)} onPaste={e=>handleImagePaste(e, newThreadAttachments, setNewThreadAttachments)} rows={4} placeholder="อธิบายปัญหาที่พบ... (Paste รูปได้เลย)"
            ...
            <div style={{ marginBottom:'20px' }}>
              ...
              <SupportAttachDropZone attachments={newThreadAttachments} setAttachments={setNewThreadAttachments} maxImages={5}/>
            </div>

            <div style={{ display:'flex', justifyContent:'flex-end', gap:'8px' }}>
              <button onClick={()=>setShowNewThreadForm(false)} disabled={creatingThread}
