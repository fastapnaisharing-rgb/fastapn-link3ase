...
function InvoiceDetailPopup({ show, onClose, form, setField, vendorInfo, itemcodeItems = [], fetchCollection, userName = '', currentUser, bu = '' }) {
  ...
  const [line1, setLine1] = useState({
    hl: 'H', itemCode: '', amount: '', tax: '', taxCode: '', whtCode: '', account: '',
    desc: '', vat: '', wht: '', total: '',
  });
      ...
      <div style={{ background: 'white', ...popupStyle, display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 20px 60px rgba(26,58,92,0.22)' }}>
        ...
        <div style={{ ...card, width: '49%', marginBottom: 0 }}>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'nowrap', alignItems: 'flex-end', overflowX: 'auto', marginBottom: '14px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '3px', flex: '1 1 120px', minWidth: '120px' }}>
              ...
              <input type="text" value={form?.backDesc1 || ''} onChange={e => setField('backDesc1', e.target.value)}
                style={{ height: '30px', padding: '0 8px', fontSize: '12px', borderRadius: '6px', outline: 'none', border: '0.5px solid #ddd', background: 'white', color: '#1a3a5c', boxSizing: 'border-box', width: '100%' }} />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '3px', flex: '1 1 120px', minWidth: '120px' }}>
...
function InvoiceHeader({ form, setField, onSupplierBlur, vendorInfo, vendorLoading, matchedRule, onBranchSearch, onBranchNoChange, onBranchNoBlur, onInvoiceDetail }) {
  ...
  const fld = (label, key, opts = {}) => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '3px', width: opts.width || 'auto' }}>
      <label style={{ fontSize: '11px', color: '#888', textAlign: 'center', whiteSpace: 'nowrap' }}>
