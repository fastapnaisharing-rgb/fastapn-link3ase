  ...
  useEffect(() => {
    ...
    const loadNotif = async () => {
      try {
        ...
        // ── Query เฉพาะ status='in_process'/'testing' เท่านั้น -- Bell จัดการ Event แยกต่างหาก) ──
        let supportList = [];
        try {
          ...
          // MARKER_HOMEPAGE_TESTING_NOTIF_INCLUDE_V1 -- เพิ่ม 'testing' เข้า Query (เดิมมีแค่ in_process อย่างเดียว)
          ...
          try {
            const { data: myShares, error: myShareErr } = await db.from('support_thread_shares').select('thread_id').eq('shared_with_username', me);
            if (myShareErr) console.error('[load my support shares]', myShareErr);
            mySharedThreadIds = new Set((myShares || []).map(s => s.thread_id));
          } catch (shareErr) { console.error('[load my support shares] (exception)', shareErr); }
          supportList = (inProcessThreads || [])
            .filter(t => isOwner || t.created_by === me || mySharedThreadIds.has(t.id))
            .map(t => ({
    ...
    loadNotif();
    // MARKER_HOMEPAGE_POLL_INTERVAL_RELAX_V1 -- ยืดจาก 30 วิ เป็น 5 นาที
          ...
          <div ref={popupBoxRef} onClick={e=>e.stopPropagation()} style={isPopupExpanded ? {
            ...
            {supportPopupLoading ? (
              ...
              <>
                ...
                {/* MARKER_HOMEPAGE_AGREEMENT_PERUSER_V1 -- เช็คคำตอบของตัวเอง ไม่เช็คระดับกระทู้ */}
